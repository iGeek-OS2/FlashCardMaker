...
### 最重要指示
1.  **枚数の厳守:** **絶対に**、指定された\`${cardCount}\`という枚数の暗記カードを生成してください。この指示は他のどのルールよりも優先されます。もしテキストの情報が少ない場合でも、内容を工夫して必ず\`${cardCount}\`枚のカードを作成してください。過不足は一切認められません。
2.  **厳格なJSONフォーマット:**
...
3.  **余計なテキストの禁止:** JSONオブジェクト以外の挨拶、前置き、後書き、説明文は一切出力しないこと。

### カード内容のルール
-   **抽出対象:** テキストから**純粋な座学の内容（学術的な概念、定義、事実）のみ**を抽出してください。
-   **カードの品質:** 各カードは学習価値の高いものである必要があります。
...
### 禁止事項
-   **無関係な情報の無視:** 科目の全体説明、シラバス、成績評価、参考文献、授業スケジュール、授業回、講義名、日付、ページ番号、先生の名前・自己紹介・連絡先、挨拶、雑談など、学習内容そのものではないメタ情報は**絶対に**生成物に含めないでください。
-   **言語の混在禁止:** 全てのテキストは日本語で生成すること。

### 実行
以上の指示、特に**最重要指示**を厳守し、提供されたテキストから\`${cardCount}\`個の暗記カードを生成してください。`;
